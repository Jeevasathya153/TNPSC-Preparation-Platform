...
const Contest = () => {
  ...
  const fetchContest = useCallback(async () => {
    try {
      ...
      const endpoint = activeTab === 'daily' ? '/contests/daily' : '/contests/weekly';
      ...
      if (response.data?.id && user?.id) {
        ...
        const participationRes = await apiClient.get(
          `/contests/${response.data.id}/check-participation/${user.id}`
        );
        ...
        const leaderboardRes = await apiClient.get(
          `/contests/${response.data.id}/leaderboard`
        );
        setLeaderboard(leaderboardRes.data || []);

        // Fetch stats
        const statsRes = await apiClient.get(
          `/contests/${response.data.id}/stats`
        );
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      // Refresh leaderboard
      const leaderboardRes = await apiClient.get(
        `/contests/${contest.id}/leaderboard`
      );
      setLeaderboard(leaderboardRes.data || []);
