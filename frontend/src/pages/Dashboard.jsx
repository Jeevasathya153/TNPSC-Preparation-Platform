...
const ContestModal = ({ isOpen, onClose, contestType, user, onContestComplete }) => {
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      const endpoint = contestType === 'daily' ? '/contests/daily' : '/contests/weekly';
      ...
      const response = await apiClient.get(`/contests/${contest.id}/leaderboard`);
  ...
  const handleSubmitContest = async () => {
    ...
    try {
      ...
      const contestEndpoint = contestType === 'daily' ? '/contests/daily' : '/contests/weekly';
      ...
      if (currentContest?.id) {
        await apiClient.post(`/contests/${currentContest.id}/submit`, {
          userId,
          userEmail,
  ...
  const fetchContestLeaderboards = async () => {
    try {
      ...
      const [dailyContestRes, weeklyContestRes] = await Promise.all([
        apiClient.get('/contests/daily').catch(() => ({ data: null })),
        apiClient.get('/contests/weekly').catch(() => ({ data: null }))
      ]);
      ...
      const [dailyLeaderboardRes, weeklyLeaderboardRes] = await Promise.all([
        dailyContest?.id 
          ? apiClient.get(`/contests/${dailyContest.id}/leaderboard`).catch(() => ({ data: [] }))
          : Promise.resolve({ data: [] }),
        weeklyContest?.id 
          ? apiClient.get(`/contests/${weeklyContest.id}/leaderboard`).catch(() => ({ data: [] }))
          : Promise.resolve({ data: [] })
      ]);
